...
import { FaTasks } from "react-icons/fa"; // آیکون تسک‌ها رو عوض کردم که با بقیه هماهنگ باشه

// لیست آیتم‌های نویگیشن برای مدیریت راحت‌تر
const navItems = [
  { path: "/", labelKey: "tap", icon: PiHandTapFill },
  { path: "/mine", labelKey: "mine", icon: PiHammer }, // فرض می‌کنیم روت mine هم داری
  // { path: "/earn", labelKey: "earn", icon: RiCopperCoinLine }, // فرض می‌کنیم روت earn هم داری
  { path: "/friends", labelKey: "friends", icon: RiTeamLine },
  { path: "/tasks", labelKey: "tasks", icon: FaTasks },
];
...
const BottomNav = () => {
  ...
  return (
    <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-xl z-50">
      ...
      <div className="relative flex justify-around items-center p-2 bg-black/30 backdrop-blur-lg border border-white/10 rounded-full shadow-lg">
        {navItems.map((item) => (
          <NavLink
            to={item.path}
            key={item.path}
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {/* متن */}
                <span className="text-xs font-bold relative">{t(item.labelKey)}</span>
              </>
            )}
          </NavLink>
        ))}
